...
class TLDetector(object):
    def __init__(self):
        ...
        # Camera image data from the car
    ...
    def process_traffic_lights(self):
        ...
        """
        light = None
        # Closest traffic light to car
        closest_light = None
        # Waypoint index for stop line corresponding to traffic light
        line_wp_idx = None
        ...
        if(self.pose):
            ...
            # Start off with max difference
            diff = len(self.waypoints.waypoints)
            # Traffic light list relatively small, so just iterate over it
            for i, light in enumerate(self.lights):
                # Get stop line waypoint index
                line = stop_line_positions[i]
                temp_wp_idx = self.get_closest_waypoint(line[0], line[1])
                # Find the difference between waypoints associated with the car 
                # and traffic light stop line
                d = temp_wp_idx - car_wp_idx
                # Ensure waypoint is in front of car
                if d >= 0 and d < diff:
                    diff = d
                    closest_light = light
                    line_wp_idx = temp_wp_idx

        # Found the closest traffic light
        if closest_light:
